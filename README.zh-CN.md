...
Codex Gallery 是一个本地优先的桌面图库应用，用来浏览 Codex 生成的图片。它会扫描本地 Codex 数据目录，按对话会话归类图片，在能读取到会话信息时展示会话标题，并支持收藏、打标签、对比、预览、查看元信息和导出图片。
...
- **标签**：给图片添加标签，在左侧按标签浏览图片，并在独立标签页中重命名或删除标签。
- **缩略图缩放**：通过工具栏按钮或平滑的 `Control` + 鼠标滚轮调整图片列表尺寸。
- **多选**：在图片网格中选择多张图片，并批量导出。
- **图片对比**：把最多四张图片加入对比托盘，并用网格、横向、纵向或叠层分割视图查看。
- **大图预览**：点击图片后居中大图预览，并支持上一张/下一张。
- **图片元信息**：查看文件名、尺寸、大小、格式、路径、会话 id、项目目录和修改时间。
- **导出图片**：将单张或选中的图片复制到 Downloads 目录，保留原始文件名，不移动原图。
...
![使用演示数据生成的 Codex Gallery 时间线截图](./docs/assets/codex-gallery-screenshot.png)

这张截图只使用生成的演示数据，不包含本地 Codex 会话标题、图片文件或聊天元数据。
...
- 标签数据保存在应用自己的 `codex-gallery.db` 中。
- 缩略图按需生成，并缓存在应用数据目录。
...
pnpm lint             # 运行 ESLint
pnpm release:check    # 检查 release tag 和应用版本是否一致
pnpm release:bump     # 同步更新 package、Tauri、Cargo.toml 和 Cargo.lock 版本
pnpm release:tag      # 根据当前应用版本创建 release tag
...
使用 release helper，避免 `package.json`、`src-tauri/tauri.conf.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 和 tag 版本不一致：
...
如果要指定明确版本，可以运行 `pnpm release:bump -- 1.0.1`。`pnpm release:tag` 会在工作区不干净或 tag 已存在时拒绝创建 tag。
...
├── scripts/                # 工具脚本，包括 release 版本辅助脚本
├── docs/                   # README 截图和文档资源
└── public/                 # 静态资源
```
...
- 增加缩略图缓存清理。
- 增加签名和 notarization。
- 补充扫描、Missing Session、标签、对比和导出相关测试。
